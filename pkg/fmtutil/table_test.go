...
func TestTablePlainRowTo(t *testing.T) {
	// plain 模式下 To(w) 指定目标: 流出行进入 w 而不是 stdout
	var buf bytes.Buffer
	tb := NewTable("A", "B").PlainRowLimit(2).To(&buf)
	for i := 0; i < 5; i++ {
		tb.AddRow(Cf("%d", i), C("x"))
	}
	tb.Render() // plain 模式空操作
	out := buf.String()
	lines := strings.Split(strings.TrimRight(out, "\n"), "\n")
	if len(lines) != 6 {
		t.Fatalf("expected tsv header + 5 rows, got %d:\n%q", len(lines), out)
	}
	if lines[0] != "A\tB" || lines[1] != "0\tx" || lines[5] != "4\tx" {
		t.Errorf("plain rows wrong:\n%q", out)
	}
	if tb.Len() != 5 {
