// s3iface_types.go 将 api 的对外 DTO 类型别名到中立的 s3iface 包.
//
// s3iface 拥有类型定义; api 通过别名引用, 使得:
//   - api.Client 的方法签名使用 s3iface 类型, 从而结构化满足 s3iface.S3Operations 接口
...
type UploadPartCopyOptions = s3iface.UploadPartCopyOptions
type UploadPartCopyOutput = s3iface.UploadPartCopyOutput
...
// ---- 公共访问阻断 / Object Lock / 复制 / ACL / 恢复 ----
type PublicAccessBlockConfiguration = s3iface.PublicAccessBlockConfiguration
type ObjectLockConfiguration = s3iface.ObjectLockConfiguration
type ObjectLockRule = s3iface.ObjectLockRule
type DefaultRetention = s3iface.DefaultRetention
type ObjectLockRetention = s3iface.ObjectLockRetention
type ObjectLockLegalHold = s3iface.ObjectLockLegalHold
type RestoreRequest = s3iface.RestoreRequest
type GlacierJobParameters = s3iface.GlacierJobParameters
type ReplicationConfiguration = s3iface.ReplicationConfiguration
type ReplicationRule = s3iface.ReplicationRule
type ReplicationFilter = s3iface.ReplicationFilter
type ReplicationDestination = s3iface.ReplicationDestination
type ACLOptions = s3iface.ACLOptions
